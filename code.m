...
[b, Fs]=audioread('1p.wav'); %Audio File, Fs is sampling rate
% sound(b,Fs);
...
[peak,loc] = findpeaks(y,x_axis3);
plot(x_axis3,y,loc,peak,'o');
title('VOP Evidence Plot');


%...........Finding voice regiond in speech signal........
[peak,loc] = findpeaks(y,x_axis3);
% figure,plot(x_axis3,y,loc,peak,'o');

y2 = 1-y;
[peak2,loc2] = findpeaks(y2,x_axis3);
% figure,plot(x_axis3,y2,loc2,peak2,'o');

[h,w] = size(loc);
succdist = zeros(h,w);
for i = 1:w-1
    succdist(1,i) = loc(1,i+1) - loc(1,i);
end
% figure, plot(succdist);
% [h2,w2] = size(loc2);
% W = min(w,w2);
% peaks = zeros(h,2*W);
% locations = zeros(h,2*W);
% for i=2:2:2*W
%     peaks(1,i) = peak(i/2,1);
%     locations(1,i) = loc(1,i/2);
%     peaks(1,i+1) = peak2(i/2,1);
%     locations(1,i+1) = loc2(1,i/2);
% end
% figure,plot(locations,peaks,'o');